...
const ASTRA=(()=>{
  ...
  return{
    ...
    earn(n){save.bank+=n;commit();},
    pay(n){if(save.bank<n)return false;save.bank-=n;commit();return true;},
    transfer(n){n=Math.max(0,Math.min(n,save.bank));save.bank-=n;save.carried+=n;commit();return n;},
    setCarried(n){if(save.carried!==n){save.carried=n;commit();}},
    onDeath(pos,items){save.bank+=save.carried;save.carried=0;save.death={x:pos.x,y:pos.y,items:items||[],ts:Date.now()};commit();},
    recover(){const d=save.death;save.death=null;commit();return d;}
  };
})();
